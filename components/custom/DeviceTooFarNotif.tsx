...
const DeviceTooFarNotif: React.FC<DeviceTooFarNotifProps> = ({
  ...
  return (
    <div
      className="absolute top-0 left-0 flex items-center justify-center w-full h-16 gap-3 text-white shadow cursor-pointer bg-red_info"
      onClick={(e) => {
        e.preventDefault();
        setShowSettingsWithEnd(true, true);
      }}
    >
      <p className={twMerge("text-base", jsoFont)}>DEVICE IS GETTING TOO FAR</p>
